...
log_model_run <- function(method_name, run_name, extra_params = list()) {
  ...
  mlflow_log_param("task", "binary classification")
  mlflow_log_param("features", "AST,ALT,GGT,BIL,ALP,ALB,CHOL,CHE,CREA,Age,Sex")

  # extra parametri (npr. ntree, maxdepth)
  ...
  # ako je prec NA (nema pozitivnih predikcija), stavi 0 da MLflow ne pukne
...
#RandomForest
cm_rf <- log_model_run("rf", "RandomForest-baseline", extra_params=list(ntree=50))

#Logistic Regression
# caret::train za glm: prosledi family kroz train() ... najsigurnije kroz train(..., family=binomial)
log_model_run_glm <- function() {
  try(mlflow_end_run(), silent = TRUE)
  mlflow_start_run()
  mlflow_set_tag("mlflow.runName", "LogisticRegression-baseline")

  mlflow_log_param("model", "glm")
  mlflow_log_param("task", "binary classification")
  mlflow_log_param("family", "binomial")
  mlflow_log_param("features", "AST,ALT,GGT,BIL,ALP,ALB,CHOL,CHE,CREA,Age,Sex")
...
cm_lr <- log_model_run_glm()

#Decision Tree
cm_dt <- log_model_run("rpart", "DecisionTree-baseline", extra_params=list())
...
![Slika 1: Lista eksperimenata u okviru MLFlow projekta hepatitisC_prediction](lista-runs.png)

![Slika 2: Prikaz detalja parametara i metrike koji su korišćeni](params-metrike.png)
