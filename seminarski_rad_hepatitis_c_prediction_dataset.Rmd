...
Najviše pažnje treba obratiti na GGT, jer se pokazao kao najvažniji parametar u stablu, povišene vrednosti GGT najviše ukazuju na teže oblike oštećenja jetre.
Značajne su i vrednosti ALB (niže vrednosti ukazuju na lošiju funkciju jetre), kao i ALT i AST, čije povišene vrednosti signaliziraju oštećenje jetrenih ćelija, dok BIL ima manji uticaj u ovom modelu.
Početni model daje smislen uvid u odnose između parametara i kategorija bolesti


```{r}
# ==========================================================
# LOGIČKA VALIDACIJA MEDICINSKIH PODATAKA (R)
# ==========================================================

# 1. Identifikacija logičkih grešaka (ALB vs PROT)
# Albumin ne može biti veći od ukupnih proteina
greska_alb_prot <- dataset[dataset$ALB > dataset$PROT & !is.na(dataset$ALB) & !is.na(dataset$PROT), ]

# 2. Identifikacija "Zdravih donora" sa sumnjivo visokim enzimima
# Ako je neko 'Blood Donor' (0), a AST ili ALT su mu preko 200, to je sumnjivo
sumnjivi_donori <- dataset[dataset$Category == "0" & (dataset$AST > 200 | dataset$ALT > 200), ]

# 3. Kontradikcija kod Ciroze (CHE i Category 3)
# Ciroza skoro uvek podrazumeva nizak CHE. Visok CHE kod ciroze je sumnjiv.
sumnjiva_ciroza <- dataset[dataset$Category == "3" & dataset$CHE > 10, ]

# 4. Fizički nemoguće vrednosti (nule ili negativne vrednosti)
# Medicinski parametri u ovom setu ne mogu biti 0 ili manje
nemoguce_vrednosti <- dataset[rowSums(dataset[, 5:14] <= 0, na.rm = TRUE) > 0, ]

# --- PRIKAZ REZULTATA ---

cat("\n--- IZVEŠTAJ O LOGIČKIM OUTLIERIMA ---\n")
cat("1. Broj redova gde je ALB > PROT:", nrow(greska_alb_prot), "\n")
cat("2. Broj donora sa ekstremno visokim enzimima:", nrow(sumnjivi_donori), "\n")
cat("3. Broj pacijenata sa cirozom i previsokim CHE:", nrow(sumnjiva_ciroza), "\n")
cat("4. Broj redova sa vrednostima <= 0:", nrow(nemoguce_vrednosti), "\n")

# --- VIZUELIZACIJA PRAVILA 1 (ALB vs PROT) ---

library(ggplot2)

ggplot(dataset, aes(x = PROT, y = ALB, color = Category)) +
  geom_point(alpha = 0.5) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "red") +
  labs(title = "Validacija: Albumin vs Ukupni Proteini",
       subtitle = "Tačke iznad crvene linije (ALB > PROT) su logičke greške",
       x = "Ukupni proteini (PROT)",
       y = "Albumin (ALB)") +
  theme_minimal()
```
